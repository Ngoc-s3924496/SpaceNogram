...
struct MainMenuViewIpad11: View{
    // Initialize variable
    ...
    var body: some View{
        ZStack{
            ...
            Image("Astronaut-Ipad")
                ...
                .padding(.top, 150 - astronautOffset)
                .padding(.trailing, 45)
                .onAppear(){
                    animateAstronaut()
                }
            ...
            if self.showLeaderBoard {
                ...
                LeaderBoardViewIpad11(showLeaderBoard: $showLeaderBoard, language: $language)
                    .environmentObject(GameDataStore())
            }
            if self.showOption {
                Color.black.opacity(0.3).edgesIgnoringSafeArea(.all)
                OptionViewIpad11(showOption: $showOption, language: $language, difficulty: $difficulty, music: $music, sound: $sound,  health: $health, level: $level)
            }
            if self.showProfile {
                Color.black.opacity(0.3).edgesIgnoringSafeArea(.all)
                ProfileViewIpad11(showProfile: $showProfile, language: $language)
                    .environmentObject(GameDataStore())
            }
            if self.showContinue {
                Color.black.opacity(0.3).edgesIgnoringSafeArea(.all)
                ContinueViewIpad11(language: $language, difficulty: $difficulty, health: $health, level: $level, showContinue: $showContinue)
            }
